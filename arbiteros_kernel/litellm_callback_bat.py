...
from .policy_runtime import ENGINE
_console = Console()
...
def _workspace_root() -> str:
    # 你也可以放到 env，默认 fallback
    return os.getenv("OPENCLAW_WORKSPACE_ROOT", "/home/cstributer/.openclaw/workspace")

def _abs_under_workspace(p: str) -> str:
    p = (p or "").strip()
    if not p:
        return p
    if p.startswith("/"):
        return os.path.normpath(p)
    return os.path.normpath(os.path.join(_workspace_root(), p))

def _normalize_openclaw_tool_args(tool_name: str, args: dict) -> dict:
    if not isinstance(args, dict):
        return {}
    t = (tool_name or "").strip()
    # 先只做你观察到的工具：write/read（后面可扩展）
    if t in {"write", "read"}:
        p = args.get("path")
        if not isinstance(p, str) or not p.strip():
            p = args.get("file_path")
        if isinstance(p, str) and p.strip():
            p2 = _abs_under_workspace(p)
            # 双写最大兼容：openclaw / 你自己的 policy 都能吃
            args["path"] = p2
            args["file_path"] = p2
    return args
...
def _policy_observe_untrusted_tool_results(*, request_data: dict, session_id: str) -> None:
    ...
    for tr in tool_results:
        ...
        if not isinstance(content, str) or not content.strip():
            continue
        info = details.get(tool_call_id, {}) if isinstance(tool_call_id, str) else {}
        tool_name = info.get("tool_name") if isinstance(info, dict) else None
        tool_name = tool_name.strip() if isinstance(tool_name, str) else "unknown_tool"

        # mark as untrusted if tool_name is in taint.sources (policy_engine will decide)
        try:
            ENGINE.observe_untrusted_output(
...
def _response_transform_content_only(data: dict, message_dict: dict) -> Optional[dict]:
    # tool_calls 的消息不在这里处理（tool_call policy 在 _policy_enforce_tool_calls_in_message）
    ...
    if not (isinstance(inner, dict) and "content" in inner):
        return message_dict

    category = inner.get("category", "")
    _record_stripped_category(data, category)

    # 解析出 instruction_type（REASON/PLAN/RESPOND/ASK...）
    instruction_type = _normalize_category_to_instruction_type(category)

    # session_id（device_key）用于 policy state/EFSM/taint 的 session 维度
    md = data.get("metadata") if isinstance(data, dict) else {}
    session_id = (
        md.get("arbiteros_device_key")
        if isinstance(md, dict) and isinstance(md.get("arbiteros_device_key"), str)
        else None
    )
    if not session_id:
        # 兜底：从 messages 里推导（极少数场景 metadata 不全）
        try:
            session_id = _build_device_context(data).device_key
        except Exception:
            session_id = "unknown-channel:unknown-user"

    # instruction_parsing：尽量写入 trace 文件，并把 instruction 带给 pre_instruction 以启用 security_type
    ...
    if isinstance(trace_id, str) and trace_id.strip() and InstructionBuilder is not None:
        ...
        if builder is not None:
            try:
                instruction = builder.add_from_structured_output(
                    structured={"intent": instruction_type, "content": inner["content"]}
                )
                _save_instructions_to_trace_file(trace_id, builder)
            except Exception:
                instruction = None  # Best-effort

    # ===== MUST: instruction-based enforcement happens here =====
    try:
        dec = ENGINE.pre_instruction(
            session_id=session_id,
            instruction_type=instruction_type,
            category=(instruction.get("instruction_category") if isinstance(instruction, dict) else None),
            content=inner["content"],
            instruction=instruction,
        )
    except Exception as e:
        # fail-closed 或 fail-open 由你决定；这里按 fail-closed 更安全
        block_text = f"POLICY_BLOCK instruction={instruction_type} reason=pre_instruction_error:{e!r}"
        return {**message_dict, "content": block_text, "tool_calls": None, "function_call": None}

    if not dec.allow:
        block_text = (
            f"POLICY_BLOCK instruction={instruction_type} "
            f"reason={dec.reason} trace_id={dec.trace_id}"
        )
        return {**message_dict, "content": block_text, "tool_calls": None, "function_call": None}
    # ===== END MUST =====

    # 允许：对外只返回 inner content（保持你原来的“剥 category”行为）
...
def _policy_enforce_tool_calls_in_message(*, request_data: dict, message_dict: dict) -> dict:
        """
        Kernel-only PEP:
        - Inspect assistant message.tool_calls
        - For each tool_call(name+arguments), run ENGINE.pre(...)
        - Remove BLOCKed tool_calls from outgoing message
        - If all tool_calls blocked, set tool_calls=None and inject a readable content
        """
        if not isinstance(message_dict, dict):
            return message_dict
        ...
        md = request_data.get("metadata") if isinstance(request_data, dict) else {}
        if not isinstance(md, dict):
            md = {}
        device_key = md.get("arbiteros_device_key")
        if not isinstance(device_key, str) or not device_key.strip():
            device_key = "unknown-channel:unknown-user"
        session_id = device_key.strip()
        user_key = session_id.split(":", 1)[1] if ":" in session_id else session_id
        ...
        for tc in tool_calls:
            ...
            # 关键：归一化 path / file_path，并把相对路径补成 workspace 下绝对路径
            args = _normalize_openclaw_tool_args(tool_name, args)

            # 关键：写回 tool_call，确保真正发给 openclaw 的参数被修正
            if is_args_str:
            ...
            if dec.allow:
                ...
                blocked_lines.append(
                    f"POLICY_BLOCK tool={tool_name} reason={dec.reason} trace_id={dec.trace_id}"
                )
        ...
        if kept:
            ...
            if not isinstance(out.get("content"), str) or not out.get("content"):
                out["content"] = "\n".join(blocked_lines[:3])
...
class MyCustomHandler(CustomLogger):
    ...
    async def async_pre_call_hook(
        ...
        # --- EFSM / instruction-aware: feed USER_MESSAGE to engine (dedupe by fingerprint) ---
        if context.latest_user_text:
            try:
                ENGINE.observe_user_message(
                    session_id=context.device_key,
                    text=context.latest_user_text,
                    fingerprint=context.latest_user_fingerprint,
                )
            except Exception:
                pass
        ...
        if context.reset_requested:
            ...
            try:
                ENGINE.reset_session(context.device_key)
            except Exception:
                pass
        else:
            data = _wrap_messages_with_categories(data, device_key=context.device_key)
            _policy_observe_untrusted_tool_results(
                request_data=data,
                session_id=context.device_key,
            )
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(raw_msg_dict, dict):
            raw_msg_dict = _policy_enforce_tool_calls_in_message(
                request_data=data,
                message_dict=raw_msg_dict,
            )
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        if isinstance(msg_dict, dict):
            msg_dict = _policy_enforce_tool_calls_in_message(
                request_data=request_data,
