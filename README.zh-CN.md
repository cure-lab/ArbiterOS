...
#### ArbiterOS 运行在你的 Agent 运行时之下或旁侧，在副作用发生前执行策略约束、产出权威执行轨迹，并拦截不安全动作。
...
- **权威执行轨迹**：记录 Agent 规划了什么、调用了什么、返回了什么。
- **策略执行**：对解析后的指令、工具调用和带污点传播的数据流做策略检查。
- **不安全动作拦截**：在敏感副作用真正发生前阻断风险操作。
...
这些数字需要结合各自 benchmark 的指标定义与 baseline 一起理解。以当前仓库状态来看，README 中更适合将它们作为结果概览，而不是将其表述为一个已经完整封装好的可复现实验包。
